...
export const sort = (data) => {
  const results = data.assets.map((currencys) => {
    ...
    coin.marketSymbol = market[0].marketSymbol.substr(
      market[0].marketSymbol.indexOf(":") + 1,
      market[0].marketSymbol.lenght
    );
    coin.percentChange = !market[0].ticker
      ? null
      : market[0].ticker.percentChange;
    coin.lastPrice = !market[0].ticker
      ? null
      : Number(market[0].ticker.lastPrice).toFixed(2).toString();
    coin.lowPrice = !market[0].ticker
      ? null
      : Number(market[0].ticker.lowPrice).toFixed(2).toString();
    coin.highPrice = !market[0].ticker
      ? null
      : Number(market[0].ticker.highPrice).toFixed(2).toString();
    coin.baseVolume = !market[0].ticker ? null : market[0].ticker.baseVolume;
    coin.quoteVolume = !market[0].ticker ? null : market[0].ticker.quoteVolume;
    return coin;
  });
